...
export default function PropriedadeEditModal({ isOpen, onClose, propriedade, onUpdated }) {
  ...
  useEffect(() => {
    if (propriedade && isOpen) {
      setStep(1); // Sempre abrir na etapa de endereço
      setPropriedadeForm({
        nome: propriedade.nome || "",
        cnpj: propriedade.cnpj || "",
        tipo_manejo: propriedade.tipo_manejo || "",
        p_abcb: propriedade.p_abcb || false,
      });
      setIdEndereco(propriedade.id_endereco || "");
      if (propriedade.endereco) {
        setEnderecoForm({
          pais: propriedade.endereco.pais || "",
  ...
  // Atualiza o endereço antes de atualizar a propriedade
  const handleEnderecoSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    setError("");
    try {
      await enderecoService.atualizarEndereco(propriedade.id_endereco, enderecoForm);
      setStep(2);
    } catch (err) {
      setError("Erro ao atualizar endereço. Verifique os dados e tente novamente.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePropriedadeSubmit = async (e) => {
    ...
    try {
      ...
      await propriedadeService.editarPropriedade(propriedade.id_propriedade, propriedadePayload);
      onUpdated && onUpdated();
      onClose();
    } catch (err) {
      setError("Erro ao atualizar propriedade. Verifique os dados e tente novamente.");
    } finally {
      setLoading(false);
    }
  };
        ...
        {step === 1 && (
          <form className="p-6" onSubmit={handleEnderecoSubmit}>
            ...
            <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-gray-200">
              ...
              <button
                type="button"
                className="px-4 py-2 bg-[#e0e0e0] text-gray-800 rounded-lg hover:bg-[#d1d1d1] transition-colors font-medium flex items-center gap-2"
                onClick={() => setStep(2)}
                disabled={loading}
              >
                Pular para edição de Propriedade
              </button>
              <button 
                type="submit" 
